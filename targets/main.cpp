...
void AppMain()
{
	...
	ParticleSystem *ps = new ParticleSystem(100000);

	// emitters
	ps->AddEmitter(std::make_shared<SphereEmitter>(glm::vec3(0,0,0), 1));

	// generators
	std::shared_ptr<TextureGenerator> tg = std::make_shared<TextureGenerator>();
	tg->AddTexture("../resources/textures/fuzzball.png");
	ps->AddGenerator(tg);
	ps->AddGenerator(std::make_shared<VelocityGenerator>(0.1,0.5));
